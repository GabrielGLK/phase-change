...
void lee_model(scalar tr, scalar f, scalar m, double L_h){
  ...
  foreach()
  {
    ...
    r_i[] = 0.68*tr.tr_eq/(L_h*(0.5+0.5*f[])*Delta*f[]*rho1);//mass transfer intensity factor expression // saturated thermo-conductivity, film-boiling is 40,but this does not work. it's good for stefan problem
    if(interfacial(point,f))
...
/**
# Advection with a non-solenoidal velocity, from Quentin
