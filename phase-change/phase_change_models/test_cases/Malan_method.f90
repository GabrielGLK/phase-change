...
subroutine ReadHeatParameters
  ...
  namelist /heatparameters/ Cp1, Cp2, h_fg, kc1, kc2, T_sat, T0_1, T0_2,&
       ...
       perturb_T, epsT, epsBL, EtAdvScheme, HYPRE_Ps, HYPRE_Timp !定义变量
...
subroutine initialize_heat !初始化
  use module_grid
  use module_flow    
  use module_boil
  implicit none
...
subroutine SetBoundFaceTemps(d,T_min,T_plus)!设置温度边界条件
  use module_grid
  use module_boil
  implicit none
...
subroutine dE_PhaseChange(rho1,rho2)!energy change due to phase change
  use module_grid
  use module_boil
  use module_BC
  implicit none
...
subroutine get_Te_from_energy!通过能量值获得温度场
  use module_grid
  use module_boil
  implicit none
  integer :: i,j,k
...
!Boundary values for temperature !这里是网格边界的温度Tf，为了计算energy diffusion
...
subroutine setuppoisson_phase_change(coeff)!计算entire domain里面的poisson方程系数
  use module_grid
  use module_flow
  use module_boil
  implicit none
...
subroutine setuppoisson_phase_change_ext(coeff)!计算subdomain里面的poisson方程系数
  use module_grid
  use module_flow
  use module_boil
  implicit none
...
subroutine phase_change_vel_correction!subdomain里面的速度
  use module_grid
  use module_flow
  use module_boil
  implicit none
...
subroutine get_ghost_masks
  ...
  topo_mask = 3*(2*vof_phase - 1)!将subdomain初始设置为interface向外三层
        ...
        do phase=0,1
           ...
           if (topo_mask(i,j,k)==3*phase_sign .and. vof_phase(i,j,k)==phase) then
              if ((topo_mask(i+1,j,k)==phase_sign*(level-1)).or.&
                 ...
                 topo_mask(i,j,k)=level*phase_sign !将subdomain减为界面和向外两层
              endif
           endif
        enddo
     enddo; enddo; enddo
...
subroutine sweep_mass_transfer(rho_l,dt,dh) !计算相变后的VOF
  use module_grid
  use module_boil
  use module_VOF
  use module_BC
  ...
  integer :: i,j,k
  nshift=mdot/rho_l*dt/dh!相变移动的距离
  call LevelSetShift2VOFCHANGE(nshift,dc)!相变后的vof值

end subroutine sweep_mass_transfer

subroutine NuAvg(t)!计算平均雷诺数
  use module_grid
  use module_boil
  implicit none
  include 'mpif.h'
