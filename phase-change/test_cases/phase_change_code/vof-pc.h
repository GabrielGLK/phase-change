...
The face velocity field `u_l` will be defined by a solver as well
as the timestep. */

extern scalar * interfaces;
extern face vector u_l;
extern double dt;
...
static void sweep_x (scalar c, scalar cc)
{
  ...
  foreach_face(x, reduction (max:cfl)) {
    ...
    double un = u_l.x[]*dt/(Delta*fm.x[] + SEPS), s = sign(un);
    int i = -(s + 1.)/2.;
...
//if(cf>1-1e-5||cf<1e-5)
  ...
  */
  ...
  foreach() {
    c[] += dt*(flux[] - flux[1] + cc[]*(u_l.x[1] - u_l.x[]))/(cm[]*Delta + SEPS);
    //c[] += dt*(flux[] - flux[1])/(cm[]*Delta + SEPS);
    scalar t, tflux;
