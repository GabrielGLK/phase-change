...
p_new[right] = neumann (neumann_pressure(ghost));
p_new[left]  = neumann (- neumann_pressure(0));
#if AXI
uf.n[bottom] = 0.;
...
p_new[top]    = neumann (neumann_pressure(ghost));
#else // !AXI
#  if dimension > 1
p[top]    = neumann (neumann_pressure(ghost));
p[bottom] = neumann (- neumann_pressure(0));
p_new[top]    = neumann (neumann_pressure(ghost));
p_new[bottom] = neumann (- neumann_pressure(0));
#  endif
#  if dimension > 2
p[front]  = neumann (neumann_pressure(ghost));
p[back]   = neumann (- neumann_pressure(0));
p_new[front]    = neumann (neumann_pressure(ghost));
p_new[back] = neumann (- neumann_pressure(0));
#  endif
#endif // !AXI
