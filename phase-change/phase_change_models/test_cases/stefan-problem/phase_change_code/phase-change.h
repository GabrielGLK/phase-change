  ...
  foreach()
    { 
      ...
      */
      ...
      if(interfacial(point,f)&&alpha_l<0)
        T_p = interpolate_2 (point, tr, p);
      if(n.x>0&&n.y>0)
        {
          if(interfacial(neighborp(-1,-1),f)&&f[-1,-1]==1)
            tr[] = 2*tr.tr_eq - T_p;
        }
      else if(n.x<0&&n.y<0)
        {
...
mgstats dirichlet_diffusion (struct Dirichlet_Diffusion p) {
  ...
  foreach() {
    volumic_metric[] = cm[];
    dirichlet_source_term[] = cm[]*tr.tr_eq*tr.D*time_factor*(1-f[])*sq((1<<max_level)/L0);
    dirichlet_feedback_term[] = - cm[]*tr.D*time_factor*(1-f[])*sq((1<<max_level)/L0);
  }
  foreach_face()
    diffusion_coef.x[] = fm.x[]*tr.D;
