...
void sun_model_vapor (scalar tr, scalar f, scalar temp, double L_h) 
{
  face vector grad_t[]; // gradient of temperature
  foreach()
    f[] = clamp(f[], 0., 1.);
  ...
  foreach_face() {
    ...
    if (interfacial(point,f)) {
      ...
      if (nf.x >= 0.) {
        grad_t.x[] = (fabs(nf.x)*(min(gtr.x[1,0] + gtr.x[],0)) + fabs(nf.y)*(nf.y >= 0. ? gtr.x[1, 1] : gtr.x[1, -1]));
      }
      else if (nf.x < 0.) {
        grad_t.x[] = (fabs(nf.x)*(min(gtr.x[-1,0] + gtr.x[],0)) + fabs(nf.y)*(nf.y >= 0. ? gtr.x[-1, 1] : gtr.x[-1, -1]));
      }
  ...
  foreach()
    {
      temp[] = 0;
      foreach_dimension()
          temp[] -= 0.025*grad_t.x[]*n.x[]/L_h; // unsaturated thermo-conductivity, film-boiling is 1
    }
  boundary({temp});
}


void sun_model_liquid (scalar tr, scalar f, scalar temp, double L_h) 
{
  face vector grad_t[]; // gradient of temperature
  foreach()
    f[] = clamp(f[], 0., 1.);
  ...
  foreach_face() {
    ...
    if (interfacial(point,f)) {
      ...
      if (nf.x >= 0.) {
        grad_t.x[] = (fabs(nf.x)*(gtr.x[1,0]+gtr.x[]) + fabs(nf.y)*(nf.y >= 0. ? gtr.x[1, 1] : gtr.x[1, -1]));
      }
      else if (nf.x < 0.) {
        grad_t.x[] = (fabs(nf.x)*(gtr.x[-1,0]+gtr.x[])+ fabs(nf.y)*(nf.y >= 0. ? gtr.x[-1, 1] : gtr.x[-1, -1]));
      }
  ...
  foreach()
    {
      temp[] = 0;
      foreach_dimension()
        temp[] += 0.025*grad_t.x[]*n.x[]/(sqrt(sq(n.x[]) + sq(n.y[]))*L_h); // unsaturated thermo-conductivity, film-boiling is 1
    }
  boundary({temp});
}
